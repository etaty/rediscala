...
trait Lists extends Request {
  ...
  def linsert[A](key: String, beforeAfter: String, pivot: String, value: A)(implicit convert: RedisValueConverter[A]): Future[Long] =
  ...
  def lrange(key: String, start: Long, stop: Long)(implicit convert: MultiBulkConverter[Seq[ByteString]]): Future[Try[Seq[ByteString]]] =
